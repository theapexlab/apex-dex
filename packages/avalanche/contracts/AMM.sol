...
contract AMM {
    ...
    uint256 internal totalShares; // Stores the total amount of share issued for the pool
    uint256 internal totalToken1; // Stores the amount of Token1 locked in the pool
    uint256 internal totalToken2; // Stores the amount of Token2 locked in the pool
    uint256 internal K; // Algorithmic constant used to determine price

    uint256 internal constant PRECISION = 1_000_000;

    mapping(address => uint256) internal shares; // Stores the share holding of each provider
    ...
    mapping(address => uint256) internal token1Balance;
    mapping(address => uint256) internal token2Balance;
